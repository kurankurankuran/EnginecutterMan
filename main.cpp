...
#define WINDOW_NAME		_T("エンジンカッターマン")	// ウインドウのキャプション名

#define MAX_SAVE_TEX (50)
...
bool           g_bSaveTex[MAX_SAVE_TEX];
ID3D11ShaderResourceView*         g_pSaveTex[MAX_SAVE_TEX];
...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	dwCurrentTime = dwFrameCount = 500;
...
HRESULT Init(HWND hWnd, BOOL bWindow)
{
	...
	InitSaveMesh();
	// テクスチャセーブデータ初期化
	for (int i = 0; i < MAX_SAVE_TEX; ++i) {
		g_pSaveTex[i] = nullptr;
		g_bSaveTex[i] = false;
	}
...
// テクスチャーデータ保存
//=============================================================================
void SaveTex(ID3D11ShaderResourceView* pTex)
{
	for (int i = 0; i < MAX_SAVE_TEX; i++) {
		if (g_bSaveTex[i])continue;
